...
- **SD Count** If you only have 1 SD card but multiple pis, you will have to run
  through this entire thing for each pi, individually, but it'll work.
...
1. Get the latest Ubuntu server 20.04 image from
...
1. Make sure you are downloading **AMD** architecture (you are usually and it
...
1. Go through the process of setting up the server.
   ...
   - User and password (This password can be altered, but it's just easier to
     keep it as is):
     - User: `hab`
     - Password: `vanillaHab`
   - SSH:
   ...
   - Select disk partitions and boot drives as you find appropriate, generally,
     we want at least one massive partition > 1.5 TB.
1. Once everything is installed, it will indicate a successful install, and ask
   you to eject the USB / SD media, Eject it.
1. Boot the device from disk
...
1. Once you have SSHed in, escalate the ubuntu user permissions to use sudo
   without a password
   1. run: `sudo visudo`
   1. Add: `ubuntu ALL=(ALL) NOPASSWD:ALL` to the end of the file
...
If you don't have any other computers, you can proceed to the section:
[Stand-up Live Hosts](/docs/l1-hosts/live-hosts). Otherwise, change the tab
above to the next set of hosts.
